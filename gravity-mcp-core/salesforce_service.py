...
# Global connection pool reused by all users
...
class SalesforceService:
    ...
    async def get_current_user_profile(self) -> str:
        """Fetches and caches the Profile Name of the currently authenticated user."""
        if self._profile_name is not None:
            return self._profile_name
            
        url = f"{self.instance_url}/services/oauth2/userinfo"
        try:
            response = await global_http_client.get(url, headers=self.headers)
            ...
            if not user_id:
                logger.warning("Could not extract user_id from userinfo")
                return ""
                
            soql = f"SELECT Profile.Name FROM User WHERE Id = '{user_id}'"
            result = await self.run_soql_query(soql)
            records = result.get("records", [])
            
            if records and records[0].get("Profile"):
                self._profile_name = records[0]["Profile"].get("Name", "")
            else:
                self._profile_name = ""
    ...
    async def update_record(self, object_name: str, record_id: str, fields: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = await global_http_client.patch(url, headers=self.headers, json=fields)
    ...
    async def execute_graphql(self, query: str) -> Dict[str, Any]:
        ...
        try:
            response = await global_http_client.post(url, json=payload, headers=self.headers)
    ...
    async def find_object_api_name(self, label: str) -> Dict[str, Any]:
        """
    ...
    async def fetch_records_by_ids(self, object_name: str, record_ids: list[str]) -> Dict[str, Any]:
        """
        ...
        query = f"SELECT FIELDS(ALL) FROM {object_name} WHERE Id IN ('{id_list}') LIMIT 200"
        
        logger.info("Hydrating %d %s records via SOQL", len(record_ids), object_name)
        return await self.run_soql_query(query)

    def _handle_api_error(self, action: str, error: Exception, response: Optional[httpx.Response] = None) -> Dict[str, Any]:
        """
